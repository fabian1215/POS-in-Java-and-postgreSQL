 ...
 * @author Fabian
 */
//public class ModificarProductoGUI {
//    
//}
...
public class ModificarProductoGUI extends JFrame {
  ...
  private JTextField txtPath;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModificarProductoGUI frame1 = new ModificarProductoGUI();
...
//		contentPane.setBackground(new Color(123, 104, 238));
		...
		JLabel lblIngreseLosNuevos = new JLabel("Modificar producto:");
...
//        fileChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
	...
	btnConsultar.setBounds(280, 145, 100, 24);
		...
		JLabel lblNota = new JLabel("Nota: Consulte el producto con el codigo");
	...
	private void botonCrearActionPerformed( ActionEvent evt ){
		...
		categoria = CampoCategoria.getText();
                imagen = txtPath.getText();
                  if (campoNumero.getText().equals("")==false) {
                ...
                if ((CampoCantidad.getText().equals("")==false)) {
                  ...
                  if ((CampoPrecioProveedor.getText().equals("")==false)) {
                      precioProveedor = Double.parseDouble(CampoPrecioProveedor.getText());
                  }
...
// 	JOptionPane.showMessageDialog(null,""+ nombre + direccion + numeroSede + imagen);
		...
		int numFilas = controladorProducto.modificarProducto(nombre, categoria, imagen, codigo,
                         cantidad,precioProveedor,precioVenta);
               if(numFilas>0) {
		 JOptionPane.showMessageDialog(null, "El producto ha sido Modificado.", "Producto Modificado", JOptionPane.WARNING_MESSAGE);
		 dispose();
                }
		}
		...
		String codigo;
		codigo = campoNumero.getText();
		
		Producto u;
		u = new Producto();
                if (codigo.equals("")==false) {
		u = controladorProducto.consultarProducto(Integer.parseInt(codigo));
                }
		if ( u.getNombre() == null  ) {
			JOptionPane.showMessageDialog(null, "El producto no se encuentra registrado en el Sistema", "No Encontrado", JOptionPane.INFORMATION_MESSAGE);
			btnCrear.setEnabled(false);
		} else {
                    campoNumero.setEditable(false);
                 btnCrear.setEnabled(true);
			campoNombre.setText(u.getNombre());
			CampoCategoria.setText(u.getCategoria());
                         txtPath.setText(u.getImagen());
          
       ImageIcon imageIcon = new ImageIcon(new ImageIcon(u.getImagen()).getImage().getScaledInstance(130,130, Image.SCALE_DEFAULT));
       Imagen.setIcon(imageIcon);    
			CampoCantidad.setText(Integer.toString(u.getCantidad()));
			CampoPrecioProveedor.setText(Double.toString(u.getPrecioProveedor()));
			CampoPrecioVenta.setText(Double.toString(u.getPrecioVenta()));
